...
public final class WebsocketRouteTransport extends BaseWebsocketServerTransport<WebsocketRouteTransport, Closeable> {
   ...
   */
  public static BiFunction<WebsocketInbound, WebsocketOutbound, Publisher<Void>> newHandler(ConnectionAcceptor acceptor) {
    return (in, out) -> acceptor.accept(new WebsocketConnection("server", (Connection) in))
