...
import infra.cloud.serialize.format.value.ValueFactory;
import infra.cloud.serialize.format.value.Variable;
...
public class MessageUnpacker implements Closeable {
  ...
  protected MessageUnpacker(MessageBufferInput in, MessagePack.UnpackerConfig config) {
    this.in = Objects.requireNonNull(in, "MessageBufferInput is null");
  ...
  public MessageBufferInput reset(MessageBufferInput in) throws IOException {
    MessageBufferInput newIn = Objects.requireNonNull(in, "MessageBufferInput is null");
   ...
   * Reads a short.
   * <p>
   * This method throws {@link MessageIntegerOverflowException} if the value
   * doesn't fit in the range of short. This may happen when {@link #getNextFormat()}
   * returns UINT16, INT32, or larger integer formats.
   *
   * @return the read value
   ...
   * <p>
   * This method throws {@link MessageIntegerOverflowException} if the value
   * doesn't fit in the range of int. This may happen when {@link #getNextFormat()}
   * returns UINT32, INT64, or larger integer formats.
   *
   * @return the read value
   ...
   * Reads a long.
   * <p>
   * This method throws {@link MessageIntegerOverflowException} if the value doesn't
   * fit in the range of long. This may happen when {@link #getNextFormat()} returns UINT64.
   *
   * @return the read value
   ...
   * Reads a float.
   * <p>
   * This method rounds value to the range of float when precision of the read
   * value is larger than the range of float. This may happen when {@link #getNextFormat()} returns FLOAT64.
   *
   * @return the read value
  ...
  public ExtensionTypeHeader unpackExtensionTypeHeader() throws IOException {
    ...
    switch (b) {
      ...
      case Code.EXT32: {
        ...
        int length = numberBuffer.getInt(nextReadPosition);
        if (length < 0) {
          throw overflowU32Size(length);
        }
